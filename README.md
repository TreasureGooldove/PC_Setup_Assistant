...
- 方案操作：锁定配件、从候选列表自行选择、查看规格/排行/点评/来源、刷新参考报价并实时查看生成进度。
- 清单导出：生成包含方案概览、配件明细和兼容性检查的 `.xlsx` 文件。
- 硬件天梯：按 CPU/显卡分类，用型号、品牌和价格筛选 S/A/B 档，点击型号可进入手动选配详情。
...
已有：本地演示、需求对话、三套方案、兼容性规则、持久化 Worker、SSE、生成进度条、导出、可筛选天梯、手动选配详情、游戏最低/推荐配置查询和 CI。

预留：京东联盟/拼多多多多客/淘宝联盟接口、公开商品页人工确认采集、Steam Store 更多字段标准化、用户提供的视频字幕或摘要证据、更多硬件规格和多用户存储。
