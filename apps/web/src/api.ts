...
export const api = {
  createConversation: (profile: NeedProfile) =>
    request<ConversationResponse>("/api/conversations", {
      method: "POST",
      body: JSON.stringify({ profile }),
    }),
  updateProfile: (id: string, profile: NeedProfile) =>
    request<ConversationResponse>(`/api/conversations/${id}/profile`, {
      method: "PATCH",
      body: JSON.stringify({ profile }),
    }),
  sendMessage: (id: string, content: string) =>
    request<ConversationResponse>(`/api/conversations/${id}/messages`, {
      method: "POST",
      body: JSON.stringify({ content }),
    }),
  generate: (conversationId: string) =>
    request<Job>(
      `/api/plans/generate?conversation_id=${encodeURIComponent(conversationId)}`,
      {
        method: "POST",
        headers: {
          "Idempotency-Key": `generate:${conversationId}:${Date.now()}`,
        },
      },
    ),
  getJob: (id: string) => request<Job>(`/api/jobs/${id}`),
  getPlans: (conversationId: string) =>
    request<{ plans: BuildPlan[] }>(
  ...
  getLadder: (
    ...
    return request<{ items: HardwareLadderEntry[] }>(
      `/api/ladder?${params.toString()}`,
    );
  },
  searchGames: (query: string) =>
    request<{ items: GameSearchResult[] }>(
      `/api/games/search?query=${encodeURIComponent(query)}`,
    ),
  getGameRequirements: (appId: string) =>
  ...
  replaceItem: (
    ...
    request<BuildPlan>(`/api/plans/${planId}/items/${slot}`, {
      method: "PATCH",
  ...
  exportPlan: (planId: string) =>
    request<Job>(`/api/plans/${planId}/exports`, {
      method: "POST",
      headers: { "Idempotency-Key": `export:${planId}:${Date.now()}` },
    }),
};
