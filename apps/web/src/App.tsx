...
const DEMO_PARTS: Record<string, Part> = {
  ...
  motherboard: { id: "demo-board", category: "motherboard", name: "B650M WiFi 主板", brand: "AMD平台", price: 899, source: "Fixture参考价", specs: { socket: "AM5", memory_type: "DDR5", form_factor: "mATX", m2_slots: 2, sata_ports: 4, gpu_slot: "PCIe4 x16" }, power_w: 0 },
  gpu: { id: "demo-gpu", category: "gpu", name: "GeForce RTX 4070 SUPER 12G", brand: "NVIDIA", price: 4499, source: "Fixture参考价", specs: { length_mm: 300, pcie_slot: "PCIe4 x16", power_connectors: ["12VHPWR"] }, power_w: 220 },
  ...
  storage: { id: "demo-storage", category: "storage", name: "1TB PCIe 4.0 固态硬盘", brand: "西数", price: 499, source: "Fixture参考价", specs: { capacity_gb: 1024, interface: "PCIe4", connector: "M.2" }, power_w: 0 },
  psu: { id: "demo-psu", category: "psu", name: "750W 金牌全模组电源", brand: "安钛克", price: 799, source: "Fixture参考价", specs: { wattage: 750, pcie_8pin_count: 3, atx_3_0: false, twelve_vhpwr: false }, power_w: 0 },
  cooling: { id: "demo-cooling", category: "cooling", name: "双塔风冷散热器", brand: "利民", price: 199, source: "Fixture参考价", specs: { type: "air", height_mm: 157, capacity_w: 220, supported_sockets: ["AM5", "LGA1700"] }, power_w: 0 },
  case: { id: "demo-case", category: "case", name: "通风型 ATX 机箱", brand: "乔思伯", price: 499, source: "Fixture参考价", specs: { form_factor: "ATX", supported_form_factors: ["ATX", "mATX", "Mini-ITX"], gpu_length_mm: 360, cooler_height_mm: 170, radiator_mm: 360 }, power_w: 0 },
};
...
const LOCAL_LADDER: HardwareLadderEntry[] = [
  ...
  { id: "cpu-12600kf", category: "cpu", tier: "B", rank: 4, name: "Core i5-12600KF", brand: "Intel", score: 82, power_w: 125, reference_price: 1199, source: "太平洋电脑网规格页 / Fixture参考价", note: "10核16线程，支持 DDR4 / DDR5" },
  { id: "cpu-13400f", category: "cpu", tier: "B", rank: 5, name: "Core i5-13400F", brand: "Intel", score: 78, power_w: 65, reference_price: 1099, source: "Fixture性能参考", note: "主流预算方案" },
...
const LOCAL_CATALOG: Part[] = [
  ...Object.values(DEMO_PARTS),
  {
    ...
    specs: { socket: "LGA1700", cores_threads: "10核16线程", base_clock: "3.7GHz", boost_clock: "4.9GHz", tdp: 125, integrated_graphics: false, l3_cache: "20MB", process: "Intel 7", architecture: "Alder Lake", memory_types: ["DDR4", "DDR5"], launch_year: 2021 },
    power_w: 125,
...
const LOCAL_GAMES: Record<string, GameRequirement> = {
  ...
  "236390": {
    ...
    minimum: requirement("Windows 10 64-bit", "双核 2.2 GHz", 4, "AMD Radeon 77XX / NVIDIA GeForce GTX 660", "DirectX 11", 70),
    recommended: requirement("Windows 10/11 64-bit", "Intel Core i5 或 AMD Ryzen 5 3600 及以上", 16, "NVIDIA GeForce GTX 1060 / AMD Radeon RX 570 及以上", "DirectX 12", 95),
    notes: "来自 Steam 商店公开系统需求；联机体验还会受分辨率、画质与网络状况影响。",
  },
};
...
function searchLocalGames(query: string) {
  ...
  return Object.values(LOCAL_GAMES)
    .filter((game) => !needle || game.app_id === query || [game.name, ...(GAME_ALIASES[game.app_id] ?? [])].some((name) => normaliseSearchText(name).includes(needle)))
...
function demoPlans(profile: NeedProfile): BuildPlan[] {
  ...
  const formFactor = profile.form_factor === "Mini-ITX" ? "Mini-ITX" : profile.form_factor === "ATX" ? "ATX" : "mATX";
  const board = { ...DEMO_PARTS.motherboard, id: `demo-board-${formFactor}`, name: `${formFactor} WiFi 主板`, specs: { ...DEMO_PARTS.motherboard.specs, form_factor: formFactor } };
  const casePart = formFactor === "Mini-ITX"
    ? { ...DEMO_PARTS.case, id: "demo-case-itx", name: "小钢炮 Mini-ITX 机箱", specs: { ...DEMO_PARTS.case.specs, form_factor: "Mini-ITX", supported_form_factors: ["Mini-ITX"], gpu_length_mm: 305, cooler_height_mm: 155, radiator_mm: 240 } }
    : formFactor === "ATX"
      ? DEMO_PARTS.case
      : { ...DEMO_PARTS.case, id: "demo-case-matx", name: "紧凑型 mATX 机箱", specs: { ...DEMO_PARTS.case.specs, form_factor: "mATX", supported_form_factors: ["mATX", "Mini-ITX"], gpu_length_mm: 330, cooler_height_mm: 165, radiator_mm: 240 } };
  const baseParts = { ...DEMO_PARTS, motherboard: board, case: casePart };
  return styles.map((style, index) => {
    const items: BuildItem[] = Object.values(baseParts).map((part) => ({ slot: part.category, part, locked: false, reason: `${part.category === "gpu" ? profile.resolution : "预算与兼容性"} 取向。` }));
...
function localInterpret(content: string, current: NeedProfile): NeedProfile {
  ...
  if (/mini[- ]?itx|itx|迷你机|小钢炮/i.test(content)) next.form_factor = "Mini-ITX";
  else if (/m[- ]?atx|matx|micro[- ]?atx|小机箱|紧凑/i.test(content)) next.form_factor = "mATX";
...
function manualPreviewIssues(items: BuildItem[]): CompatibilityIssue[] {
  ...
  const spec = (slot: PartCategory, key: string) => bySlot.get(slot)?.specs[key];
  const warning = (code: string, title: string, detail: string, related_slots: PartCategory[]): CompatibilityIssue => ({ code, severity: "warning", title, detail, related_slots });
  const error = (code: string, title: string, detail: string, related_slots: PartCategory[]): CompatibilityIssue => ({ code, severity: "error", title, detail, related_slots });
  const issues: CompatibilityIssue[] = [];

  const cpuSocket = spec("cpu", "socket");
  const boardSocket = spec("motherboard", "socket");
  if (!cpuSocket || !boardSocket) issues.push(warning("CPU_SOCKET_UNKNOWN", "CPU / 主板插槽待确认", "缺少插槽字段，生成方案后需要补充核对。", ["cpu", "motherboard"]));
  else if (cpuSocket !== boardSocket) issues.push(error("CPU_SOCKET_MISMATCH", "CPU / 主板插槽不兼容", `处理器为 ${String(cpuSocket)}，主板为 ${String(boardSocket)}。`, ["cpu", "motherboard"]));

  const memoryType = spec("memory", "memory_type");
  const boardMemory = spec("motherboard", "memory_type");
  if (!memoryType || !boardMemory) issues.push(warning("MEMORY_TYPE_UNKNOWN", "内存代际待确认", "缺少内存或主板代际字段。", ["memory", "motherboard"]));
  else if (memoryType !== boardMemory) issues.push(error("MEMORY_TYPE_MISMATCH", "内存代际不兼容", `内存为 ${String(memoryType)}，主板支持 ${String(boardMemory)}。`, ["memory", "motherboard"]));

  const boardForm = spec("motherboard", "form_factor");
  const caseForms = spec("case", "supported_form_factors");
  if (!boardForm || !Array.isArray(caseForms)) issues.push(warning("FORM_FACTOR_UNKNOWN", "机箱尺寸待确认", "缺少主板或机箱板型字段。", ["motherboard", "case"]));
  else if (!caseForms.includes(boardForm)) issues.push(error("FORM_FACTOR_MISMATCH", "主板无法安装进机箱", `机箱不支持 ${String(boardForm)} 主板。`, ["motherboard", "case"]));

  const gpuLength = Number(spec("gpu", "length_mm"));
  const caseGpuLength = Number(spec("case", "gpu_length_mm"));
  if (!gpuLength || !caseGpuLength) issues.push(warning("GPU_LENGTH_UNKNOWN", "显卡长度待确认", "缺少显卡长度或机箱限长字段。", ["gpu", "case"]));
  else if (gpuLength > caseGpuLength) issues.push(error("GPU_LENGTH_EXCEEDED", "显卡长度超限", `显卡 ${gpuLength}mm，机箱限长 ${caseGpuLength}mm。`, ["gpu", "case"]));
  ...
  if (coolingType === "air" && (!coolerHeight || !caseCoolerHeight)) issues.push(warning("COOLER_HEIGHT_UNKNOWN", "风冷高度待确认", "缺少散热器高度或机箱限高字段。", ["cooling", "case"]));
  else if (coolingType === "air" && coolerHeight > caseCoolerHeight) issues.push(error("COOLER_HEIGHT_EXCEEDED", "风冷高度超限", `散热器 ${coolerHeight}mm，机箱限高 ${caseCoolerHeight}mm。`, ["cooling", "case"]));

  const radiator = Number(spec("cooling", "radiator_mm"));
  const caseRadiator = Number(spec("case", "radiator_mm"));
  if (coolingType === "water" && (!radiator || !caseRadiator)) issues.push(warning("RADIATOR_SIZE_UNKNOWN", "冷排尺寸待确认", "缺少冷排或机箱支持尺寸字段。", ["cooling", "case"]));
  else if (coolingType === "water" && radiator > caseRadiator) issues.push(error("RADIATOR_SIZE_EXCEEDED", "冷排尺寸超限", `冷排 ${radiator}mm，机箱支持到 ${caseRadiator}mm。`, ["cooling", "case"]));

  const sockets = spec("cooling", "supported_sockets");
  if (!Array.isArray(sockets) || !cpuSocket) issues.push(warning("COOLER_SOCKET_UNKNOWN", "散热器扣具待确认", "缺少散热器支持平台字段。", ["cooling", "cpu"]));
  else if (!sockets.includes(cpuSocket)) issues.push(error("COOLER_SOCKET_MISMATCH", "散热器扣具不兼容", `散热器不支持 ${String(cpuSocket)}。`, ["cooling", "cpu"]));
  ...
  if (!storageConnector) issues.push(warning("STORAGE_INTERFACE_UNKNOWN", "硬盘接口待确认", "缺少硬盘物理接口字段。", ["storage", "motherboard"]));
  else if (storageConnector === "M.2" && !m2Slots) issues.push(error("STORAGE_INTERFACE_MISMATCH", "主板缺少 M.2 插槽", "当前硬盘需要 M.2 插槽。", ["storage", "motherboard"]));
  else if (storageConnector === "SATA" && !sataPorts) issues.push(error("STORAGE_INTERFACE_MISMATCH", "主板缺少 SATA 接口", "当前硬盘需要 SATA 接口。", ["storage", "motherboard"]));

  const powerConnectors = spec("gpu", "power_connectors");
  if (!Array.isArray(powerConnectors)) issues.push(warning("GPU_POWER_CONNECTOR_UNKNOWN", "显卡供电待确认", "缺少显卡供电接口字段。", ["gpu", "psu"]));
  else if (powerConnectors.includes("12VHPWR") && !spec("psu", "twelve_vhpwr")) issues.push(warning("GPU_POWER_CONNECTOR_ADAPTER", "显卡供电需转接", "显卡需要 12VHPWR，当前电源未标记原生接口。", ["gpu", "psu"]));

  const wattage = Number(spec("psu", "wattage"));
  const estimatedPower = items.reduce((sum, item) => sum + item.part.power_w, 80);
  if (!wattage) issues.push(warning("PSU_HEADROOM_UNKNOWN", "电源余量待确认", "缺少电源额定功率字段。", ["psu"]));
  else if (wattage < Math.ceil(estimatedPower * 1.2)) issues.push(error("PSU_HEADROOM_LOW", "电源余量不足", `预计功耗 ${estimatedPower}W，建议至少预留 20% 余量。`, ["psu"]));
  return issues;
}

function CompatibilityChecklist({ issues }: { issues: CompatibilityIssue[] }) {
  return <section className="compatibility-checklist" aria-label="兼容性检查清单">
    <div className="checklist-head"><div><span className="eyebrow">装机检查</span><h5>逐项核对</h5></div><span>{COMPATIBILITY_CHECKS.length} 项规则</span></div>
    <div className="compatibility-check-grid">{COMPATIBILITY_CHECKS.map((check) => {
      const issue = issues.find((item) => item.code === check.code || item.code.startsWith(`${check.code}_`));
      const status = issue?.severity ?? "ok";
      return <div className={`compatibility-check-row ${status}`} key={check.code}>
        {status === "error" ? <X size={15} /> : status === "warning" ? <RefreshCw size={15} /> : <CheckCircle2 size={15} />}
        <div><strong>{check.label}</strong><small>{issue?.detail ?? "已通过"}</small></div>
        <span>{status === "error" ? "需调整" : status === "warning" ? "待确认" : "通过"}</span>
      </div>;
    })}</div>
  </section>;
}
...
function RequirementColumn({ title, data }: { title: string; data: SystemRequirement }) {
  const rows = [["系统", data.operating_system], ["处理器", data.processor], ["内存", data.memory_gb ? `${data.memory_gb} GB` : "未提供"], ["显卡", data.graphics], ["DirectX", data.directx ?? "未提供"], ["存储", data.storage_gb ? `${data.storage_gb} GB 可用空间` : "未提供"], ...(data.additional_notes ? [["补充", data.additional_notes]] : [])];
...
export default function App() {
  ...
  const ladderBrands = useMemo(() => Array.from(new Set(ladderItems.map((item) => item.brand))).sort(), [ladderItems]);
  const filteredLadder = useMemo(() => {
    ...
    return ladderItems.filter((item) => {
      if (needle && !`${item.name} ${item.brand} ${item.note}`.toLowerCase().includes(needle)) return false;
      if (ladderBrand !== "all" && item.brand !== ladderBrand) return false;
      if (low !== undefined && (item.reference_price ?? 0) < low) return false;
      if (high !== undefined && (item.reference_price ?? 0) > high) return false;
      return true;
    });
  }, [ladderBrand, ladderItems, ladderMaxPrice, ladderMinPrice, ladderQuery]);
  const groupedLadder = useMemo(() => ["S", "A", "B", "C"].map((tier) => ({ tier, items: filteredLadder.filter((item) => item.tier === tier) })).filter((group) => group.items.length), [filteredLadder]);
  ...
  async function openPartPicker(slot: PartCategory, initialPartId?: string) {
    setPickerSlot(slot);
    setPickerInitialPartId(initialPartId);
    setPickerItems(LOCAL_CATALOG.filter((part) => part.category === slot));
    setPickerBusy(true);
    try {
      const catalog = await api.getCatalog(slot);
      if (catalog.items.length) setPickerItems(catalog.items);
    } catch {
  ...
  function handleSwap(item: BuildItem) {
    void openPartPicker(item.slot, item.part.id);
  }
  ...
  async function handleUsePart(part: Part) {
    ...
    setPickerBusy(true);
    try {
      const currentItem = activePlan.items.find((item) => item.slot === pickerSlot);
      if (!activePlan.id.startsWith("demo-")) {
        const updated = await api.replaceItem(activePlan.id, pickerSlot, part.id, currentItem?.locked ?? false);
        setPlans((current) => current.map((plan) => plan.id === updated.id ? updated : plan));
        setNotice(`${SLOT_LABELS[pickerSlot]}已选入，兼容性已重新计算`);
      } else {
        setPlans((current) => current.map((plan) => {
          if (plan.id !== activePlan.id) return plan;
          const items = plan.items.map((item) => item.slot === pickerSlot ? { ...item, part, reason: "用户手动选择" } : item);
          return {
            ...plan,
            items,
            total_price: items.reduce((sum, item) => sum + item.part.price, 0),
            estimated_power_w: items.reduce((sum, item) => sum + item.part.power_w, 80),
            compatibility: manualPreviewIssues(items),
          };
        }));
        setNotice(`${SLOT_LABELS[pickerSlot]}已选入预览；请生成方案完成兼容性复核`);
      }
      setPickerSlot(null);
    } catch {
  ...
  async function handleLock(item: BuildItem) {
    try {
      if (!demoMode && activePlan && !activePlan.id.startsWith("demo-")) await api.replaceItem(activePlan.id, item.slot, item.part.id, !item.locked);
  ...
  const renderPicker = () => pickerSlot ? <PartPicker slot={pickerSlot} items={pickerItems} initialPartId={pickerInitialPartId} busy={pickerBusy} onClose={() => setPickerSlot(null)} onUse={handleUsePart} /> : null;
